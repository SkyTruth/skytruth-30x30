...
def test_new_parent_exisitng_pa():
    ...
    parent_left = pd.DataFrame({"rel": [None]})
    parent_right = pd.DataFrame({"rel": [{"documentId": "doc-1"}]})
    mask = relation_diff_index(parent_left, parent_right, "rel")
...
def test_changing_parent_from_knonw_to_new_pa():
    ...
    parent_left = pd.DataFrame({"rel": [{"documentId": None}]})
    parent_right = pd.DataFrame({"rel": [{"documentId": "doc-5"}]})
    mask = relation_diff_index(parent_left, parent_right, "rel")
    assert_series_equal(mask, pd.Series([True]))
...
def test_multiple_pas_one_new_parent():
    ...
    """
    parent_left = pd.DataFrame({"rel": [{"documentId": "doc-10"}, {"documentId": "doc-7"}]})
    parent_right = pd.DataFrame({"rel": [{"documentId": "doc-11"}, {"documentId": "doc-7"}]})
    mask = relation_diff_index(parent_left, parent_right, "rel")
...
def test_children_different_lengths():
    ...
    children_left = pd.DataFrame({"rel": [[{"documentId": "doc-1"}, {"documentId": "doc-2"}]]})
    children_right = pd.DataFrame({"rel": [[{"documentId": "doc-1"}]]})
...
def test_adding_net_new_child():
    ...
    children_left = pd.DataFrame({"rel": [[{"documentId": "doc-1"}, {"documentId": None}]]})
    children_right = pd.DataFrame({"rel": [[{"documentId": "doc-1"}, {"documentId": "doc-2"}]]})
    mask = relation_diff_index(children_left, children_right, "rel")
    assert_series_equal(mask, pd.Series([True]))
...
def test_multiple_pas_one_change_children():
    ...
    """
    children_left = pd.DataFrame(
        {
            "rel": [
                [{"documentId": "doc-1"}, {"documentId": "doc-3"}],
                [{"documentId": "doc-4"}, {"documentId": "doc-5"}],
            ]
        }
    )
    children_right = pd.DataFrame(
        {
            "rel": [
                [{"documentId": "doc-1"}, {"documentId": "doc-2"}],
                [{"documentId": "doc-5"}, {"documentId": "doc-4"}],
            ]
        }
    )
    mask = relation_diff_index(children_left, children_right, "rel")
...
def test_mixed_parent_vs_children_types_true():
    ...
    dict or vice-verse
    """
    parent_left = pd.DataFrame({"rel": [{"documentId": "doc-1"}]})
    children_right = pd.DataFrame({"rel": [[{"documentId": "doc-1"}]]})
...
def test_alignment_when_right_has_extra_rows():
    ...
    children_left = pd.DataFrame(
        {"rel": [[{"documentId": "doc-1"}, {"documentId": "doc-2"}]]}, index=[10]
    )
    children_right = pd.DataFrame(
        {"rel": [[{"documentId": "doc-1"}, {"documentId": "doc-2"}], [{"documentId": "doc-3"}]]},
        index=[10, 11],
    )
    mask = relation_diff_index(children_left, children_right, "rel")
...
def test_alignment_when_left_has_extra_rows():
    ...
    children_left = pd.DataFrame(
        {"rel": [[{"documentId": "doc-1"}], [{"documentId": "doc-2"}]]}, index=[0, 1]
    )
    children_right = pd.DataFrame({"rel": [[{"documentId": "doc-1"}]]}, index=[0])
    mask = relation_diff_index(children_left, children_right, "rel")
