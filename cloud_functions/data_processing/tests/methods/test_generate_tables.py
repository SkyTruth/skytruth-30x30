...
def test_pas_with_changed_deleted_new(base_entry, child, parent):
    ...
    expected_changed = deepcopy(changed_pa)
    expected_changed["id"] = second_entry["id"]
    expected_changed["area"] = round(expected_changed["area"], 2)
    ...
    assert expected_changed in changed
