...
def test_compute_country_class_areas_no_pa_returns_both_classes(binary_coral_raster):
    country_geom = box(-10, -10, 10, 10)

    result = compute_country_class_areas(
        country="USA",
...
def test_compute_country_class_areas_with_pa_filter(binary_coral_raster):
    country_geom = box(-10, -10, 10, 10)
...
def test_compute_country_class_areas_returns_none_without_include_zero_for_all_zeros(tmp_path):
...
def test_compute_country_class_areas_handles_empty_pa_gdf(binary_coral_raster):
...
def test_compute_class_areas_by_country_two_countries(binary_coral_raster):
    ...
    df = compute_class_areas_by_country(
        raster_path=binary_coral_raster,
        regions_gdf=regions,
        class_map=CORAL_CLASS_MAP,
        region_col="location",
    ...
    assert set(df["country"]) == {"USA", "MEX"}
    usa = df[df["country"] == "USA"].iloc[0]
    mex = df[df["country"] == "MEX"].iloc[0]
...
def test_compute_class_areas_by_country_requires_polygon_country_col(binary_coral_raster):
    regions = gpd.GeoDataFrame(
        {"location": ["USA"], "geometry": [box(-10, -10, 10, 10)]}, crs="EPSG:4326"
    )
    pas = gpd.GeoDataFrame(geometry=[box(0, 0, 1, 1)], crs="EPSG:4326")

    with pytest.raises(ValueError, match="polygon_country_col"):
        compute_class_areas_by_country(
            raster_path=binary_coral_raster,
            regions_gdf=regions,
            class_map=CORAL_CLASS_MAP,
            polygons_gdf=pas,
            polygon_country_col=None,
            n_jobs=1,
            verbose=False,
        )
